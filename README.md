...
```sh
	git clone --recursive git@github.com:signaloid/Signaloid-Demo-Metallurgy-BrownHamModel.git
```
...
make local-build
```
This produces the `demo-native-mc` binary in the repository root.
2. Run the application in the Monte Carlo mode, using the (`-M`) command-line option
together with (`-S`) to select a single output:
```
...
Usage: Valid command-line arguments are:
        ...
        [-S, --select-output <output : int (Default: 1)>] (Compute 0-indexed output. Calculate all possible outputs if equal to 1.)
